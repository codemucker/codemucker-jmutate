...
/**
 * Uplifted from org.eclipse.jdt.core.dom.DefaultASTVisitor as that is package
 * private
 */
public class BaseASTVisitor extends ASTVisitor {

	static final String SEPS_FORWARD = ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>";
	static final String SEPS_BACK = "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<";
	...
	public final void preVisit(ASTNode node) {
		if( debug){
			System.out.println(SEPS_FORWARD.substring(0,depth) + " node:" + node.getClass().getName());
	...
	public final void postVisit(ASTNode node) {
		if( debug){
			System.out.println(SEPS_BACK.substring(0,depth) + " node:" + node.getClass().getName());
			depth++;
		}
	 ...
	 *
	 */
	public BaseASTVisitor(boolean visitDocTags) {
		super(visitDocTags);
	}
	...
	@Override public boolean visit(Javadoc node) {
		// do not visit Javadoc tags by default. Use constructor with boolean to
		// enable.
		if (super.visit(node)) {
			return visitNode(node);
		}
